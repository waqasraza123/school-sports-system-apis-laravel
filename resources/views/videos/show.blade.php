...
@section('content')
    <div class="container-fluid">
        <div class="row" style="margin-left: 5%; margin-right: 5%">
            ...
            <a href="/videos/add" class="btn btn-info">Add Video</a>
                ...
                <table class="table">
                    ...
                    @foreach($gallery_videos as $video)
                        <tr>
                            ...
                            <td>@foreach($video->students as $student_tag) <label>{{$student_tag->name}}
                                    , </label> @endforeach
                            </td>
                            <td>@foreach($video->rosters as $roster_tag) <label>{{$roster_tag->name}}
                                    , </label> @endforeach
                            </td>
                            <td>
                                <button data-id="{{ $video->id}}" data-toggle="modal" class="edit_video btn btn-info"
                                        data-target="#videoModal" align="right">edit
                                </button>
                            </td>
                            <td>{!! Form::open([    'method' => 'DELETE','route' => ['videos.destroy', $video->id]]) !!}{!! Form::submit('Delete', array('class' => 'btn btn-danger')) !!}{!! Form::close() !!}</td>
                            <td><p class="video_students_ids"
                                   style="display: none;">{{ json_encode($video->students->lists('id'))}} </p></td>
                            <td><p class="video_rosters_ids"
                                   style="display: none;">{{ json_encode($video->rosters->lists('id'))}} </p></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
